...
export const CharacterAvatar: React.FC<CharacterAvatarProps> = ({
  ...
  return (
    <div className={`relative inline-flex items-center justify-center flex-shrink-0 ${className}`}>
      ...
      {/* Main Avatar Body */}
      <div
        className={`relative ${sizeClasses[size]} rounded-2xl overflow-hidden bg-gradient-to-br ${character.avatarGradient} border ${
      ...
      {showBadge && (
        <span
          className="absolute -bottom-1.5 -right-1.5 px-1 py-0.2 rounded font-mono text-[9px] font-bold border border-white/20 shadow-sm"
          style={{
            backgroundColor: "#111013",
            color: character.accentColor,
          }}
        >
          {character.callsign.split("-")[0]}
        </span>
      )}
    </div>
  );
